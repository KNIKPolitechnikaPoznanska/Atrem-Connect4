			...
			public void checkIfWin() {
				...
				for (int row = 0; row < rows; row++) {
					for (int slot = 0; slot < slots - 3; slot++) {
						...
						if (token.getNumber() > 0 && token == board.getHoleState(row, slot + 1)
				...
				for (int slot = 0; slot < slots; slot++) {
					for (int row = 0; row < rows - 3; row++) {
						HoleState token = board.getHoleState(row, slot);
						if (token.getNumber() > 0 && token == board.getHoleState(row+1, slot)
				...
				for (int row = 0; row < rows - 3; row++) {
					for (int slot = 0; slot < slots - 3; slot++) {
						HoleState token = board.getHoleState(row, slot);
						if (token.getNumber() > 0 && token == board.getHoleState(row+1, slot+1)
